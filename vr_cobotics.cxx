...
vr_cobotics::vr_cobotics() : node("vr_cobotics")
{
	frame_split = 0;
...
void vr_cobotics::draw(cgv::render::context& ctx)
{
	...
	// draw wireframe boxes, can be load from external file 
...
void vr_cobotics::create_gui() {
	...
	if (begin_tree_node("NNG events", 1.f)) {
		...
		connect_copy(add_button("start sync")->click, rebind(this, &vr_cobotics::sync));
		connect_copy(add_button("show state")->click, rebind(this, &vr_cobotics::show_nng_connection_status));
...
bool vr_cobotics::self_reflect(cgv::reflect::reflection_handler & rh)
{
	return	rh.reflect_member("vr_events_record_path", vr_events_record_path) &&
			...
			rh.reflect_member("remote_address", remote_address) &&
			rh.reflect_member("listen_on", listen_on); // 
...
void vr_cobotics::sync()
{
	try {
		...
		Scene selection = buildDummyScene();
		nng::view buf;
		int length = selection.ByteSize();
		void* data = nng_alloc(length);
		selection.SerializeToArray(data, length); // can also be a scene! 
...
cgv::base::object_registration<vr_cobotics> vr_cobotics_reg("");
